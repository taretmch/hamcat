...
class FunctionKSpec extends munit.FunSuite:
  ...
  test("compose : headOptionK and flattenK") {
    val headOptionAndFlattenK: FunctionK[[X] =>> List[List[X]], Option] =
      headOptionK.compose(flattenK)

    assertEquals(
      headOptionAndFlattenK(List(List(1, 2), Nil, List(3, 4))),
      Some(1)
    )
  ...
  test("andThen : flattenK and headOptionK") {
    val flattenAndHeadOptionK: FunctionK[[X] =>> List[List[X]], Option] =
      flattenK.andThen(headOptionK)

    assertEquals(
      flattenAndHeadOptionK(List(List(1, 2), Nil, List(3, 4))),
      Some(1)
    )
  ...
  test("identityK : headOptionK") {
    ...
    val headOptionIdentityK: FunctionK[List, Option]  =
      identityK.compose(headOptionK)
    val headOptionIdentityK2: FunctionK[List, Option] =
      headOptionK.compose(identityK)
