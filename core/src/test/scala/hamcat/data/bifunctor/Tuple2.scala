...
class BifunctorTuple2Spec extends AnyFlatSpec with Matchers {
  val bifunctor = Bifunctor[Tuple2]
  ...
  "Tuple2 bifunctor" should "射の合成を保存する" in {
    ...
    assert(
      bifunctor.bimap(isEven compose increment)(isEven compose strToLength)(tuple)
        ==
      (bifunctor.bimap(isEven)(isEven) compose bifunctor.bimap(increment)(strToLength))(tuple)
    )
    assert(
  ...
  it should "恒等射を恒等射へ写す" in {
    ...
    assert(bifunctor.bimap(identity[Int])(identity[String])(tuple) == identity[(Int, String)](tuple))
  }
}
