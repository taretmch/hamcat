...
![スパン圏](./images/05_span=category.png)
...
スパン圏の例としては、射影が定義されていて、図式が可換であればなんでも良いと考えられます。例えば対象として `<String, length, isEmpty>`、`<Int, increment, isOdd>`、`<(Int, Boolean), fst, snd>` などを考えることができます。1つ目は `String` に `Int` および `Boolean` への射影を持たせたもの、2つ目は `Int` に `Int` および `Boolean` への射影を持たせたもの、3つ目は `Int` と `Boolean` の積 `(Int, Boolean)` であってこの圏の終対象になるもの、です。
...
しかしながら積が複数存在することになるので、積の同型を除いて一意である性質を満たすために、これら2つは同型でなければいけません。同型射 (つまり、逆射を持つ射) は、以下のように定義できます。
...
// res9: (Int, (String, Boolean)) = (44, ("hoge", true))
isomorTupleInv((44, ("hoge", true)))
// res10: ((Int, String), Boolean) = ((44, "hoge"), true)
```

したがって、`((A, B), C)` と `(A, (B, C))` は同型で、どちらも積であります。
...
// res13: Either[String, Int] = Left(v = "Some exceptions occur")
injIntToC(44)
// res14: Either[String, Int] = Right(v = 44)
```
...
// res19: Either[String, Nothing] = Left(v = "Oh my god!")
isomorCoproductLeftInv(Left("Oh my god!"))
// res20: String = "Oh my god!"
...
// res21: Either[Nothing, String] = Right(v = "Good")
