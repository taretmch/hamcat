...
計算が有限時間で終了するかどうかを判定する仮想的な関数 `halt: Function => Boolean` を定義できるとします。ここで `Function` は任意の関数を表す仮想的な型です。

`halt` のような関数を定義できるとき、次のような関数 `m` を考えることができます。

```
m は、入力 f に対して halt(f, f) == true であれば終了しない。
m は、入力 f に対して halt(f, f) == false であれば 0 を出力して終了する。
```

このとき、 `m` に対して `m` を入力するとどうなるでしょうか？場合分けして考えてみます。

1. `m` が終了する場合：`halt(m, m) == true` です。このとき `m` は終了しないので、矛盾します。
2. `m` が終了しない場合：`halt(m, m) == false` です。このとき `m` は 0 を出力して終了するので、矛盾します。
